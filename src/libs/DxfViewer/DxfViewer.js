...
export class DxfViewer {
   ...
   */
  constructor(_renderer, render, onLoaded, options = null) {
    ...
    for (const batch of scene.batches) {
      if (
        ...
        if (!block) {
          ...
          const dxfSceneBlock = this.dxfScene.blocks.get(batch.key.blockName);
          if (dxfSceneBlock) {
            block.offset.copy(dxfSceneBlock.offset);
          }
        }
  ...
  Subscribe(eventName, eventHandler) {
    this._EnsureRenderer();
    this.eventEmitter.addListener(EVENT_NAME_PREFIX + eventName, eventHandler);
  }
  ...
  _Emit(eventName, data = null) {
    // // console.log(eventName, (data = null));
    // return;
    this.eventEmitter.emit(EVENT_NAME_PREFIX + eventName, {detail: data});
  ...
  _GenerateShaders(instanceType, pointSize) {
    ...
    return {
      ...
      fragment: `
            ...
            void main() {
                /* In GLES 100, we need to assign the result to variable gl_FragColor in stead of using keyword out on a variable */
